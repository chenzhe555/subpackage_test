...
function createCustomModuleIdFactory() {
    ...
    return path => {
        ...
        if (path.indexOf('node_modules' + pathSep + 'react-native' + pathSep + 'Libraries' + pathSep) > 0) {
            // 这里是去除路径中的'node_modules/react-native/Libraries/‘之前（包括）的字符串(没有这个的话安卓会报错:Requiring unknown module "InitializeCore". Requiring unknown module "react-native-implementation".)
...
function processCustomModuleFilter(module) {
    if (module['path'].indexOf(pathSep + 'src' + pathSep + 'goodsdetail' + pathSep) > 0) {
        return true;
    }
    return false;
}
